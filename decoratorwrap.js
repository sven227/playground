...
// In the example code a User object is decorated (enhanced) by a DecoratedUser object. 
// It extends the User with several address-based properties. 
// The original interface must stay the same, which explains why user.name is assigned to this.name. 
// Also, the say method of DecoratedUser hides the say method of User.
...
function run() {
    ...
    user.say();
    if (user===decorated) {
        log.add ("user and decorated are same object")
    } else {
        log.add("user and decorated are NOT same object")
    }
